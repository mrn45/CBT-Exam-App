...
export function Dashboard() {
  ...
  const activeMenus = (user?.role === 'Admin' ? menusAdmin : user?.role === 'Pengawas' ? menusPengawas : menusSiswa).filter(m => {
    if (m.id === 'katrol_asli' && settings?.fitur_katrol === false) return false;
    return true;
  });
